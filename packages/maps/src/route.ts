...
/** react-native-maps coordinate format (latitude/longitude). */
export type LatLngLiteral = { latitude: number; longitude: number };

/** Quadratic Bézier — gently-curved polyline that reads as a road route. */
...
export function splitPolylineAt(
  ...
  return {
    traveled: [...polyline.slice(0, idx + 1), cut],
    remaining: [cut, ...polyline.slice(idx + 1)],
  };
}

/** Convert a directions API polyline ([lat, lng][] tuples) to LatLngLiteral[]. */
export function toLatLngLiterals(raw: Array<[number, number]>): LatLngLiteral[] {
