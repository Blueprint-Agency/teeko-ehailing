...
import { disputes } from '../../db/schema/feedback-disputes';
...
export async function routes(app: FastifyInstance) {
  ...
  app.get<{ Querystring: { queue?: Queue } }>('/', async (req, reply) => {
    ...
    const rows = await db
      ...
      .from(disputes)
      .leftJoin(users, eq(users.id, disputes.raisedById))
  ...
  app.post<{ Params: { id: string }; Body: { action?: string; note?: string } }>(
    ...
    async (req, reply) => {
      ...
      if (action === 'reject') {
        const [row] = await db
          .update(disputes)
          .set({ status: 'rejected', resolutionNote: note ?? null, handledBy, resolvedAt: now })
      ...
      if (action === 'escalate') {
        const [row] = await db
          .update(disputes)
          .set({ status: 'escalated', resolutionNote: note ?? null, handledBy })
          .where(eq(disputes.id, id))
          .returning();
        return { ok: true, dispute: serialize(row!) };
      }
