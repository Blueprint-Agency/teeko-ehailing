...
export async function routes(app: FastifyInstance) {
  // POST /api/v1/driver/ratings — driver rates the rider after a completed trip.
  app.post('/', async (req, reply) => {
    if (!req.user) return reply.code(401).send({ error: 'unauthorized' });
    const body = RateBody.parse(req.body);
    try {
      return await tripsService.driverRateTrip(req.user.id, body.tripId, body.rating, body.comment);
    } catch (err) {
